...
def merge_gdf(l2a: GeoDataFrame | dict,
    ...
    """
    Merges the data of two GeoDataFrames containing GEDI L2A and L2B data. If a
    dictionary is provided, the function assumes that the keys are the names of
    the geometries and the values are dictionaries that contain the GeoDataFrames
    and the geometry of the data. The function will merge the data of matching
    geometries and return a dictionary of GeoDataFrames.
    
    Parameters
    ----------
    l2a: GeoDataFrame or dict
        GeoDataFrame containing GEDI L2A data or a dictionary of GeoDataFrames.
    l2b: GeoDataFrame or dict
        GeoDataFrame containing GEDI L2B data or a dictionary of GeoDataFrames.
    how: str, optional
...
def _compare_gdfs(gdf_1: GeoDataFrame,
        ...
        if len(gdf_1) != len(gdf_2):
            print(f"WARNING: The GeoDataFrames{key} contain different "
                  ...
                  f"\nThey will be merged on nonetheless, which may result in "
